...
async function parseBatchPlaylist() {
    ...
    if (!text) {
        showToast('请先输入或导入歌单内容', 'warning');
        return;
    }

    const songs = parsePlaylistText(text);
    if (songs.length === 0) {
        showToast('未解析到有效歌曲', 'warning');
        return;
    }
...
function renderBatchResults() {
    ...
    batchResults.forEach((item, idx) => {
        ...
        html += `
        <div class="batch-result-item${notFoundClass}${selectedClass}" data-idx="${idx}">
            <div class="batch-item-index">${indexNum}</div>
            <div class="batch-item-checkbox">
                <input type="checkbox" data-idx="${idx}" ${isSelected ? 'checked' : ''} ${!isFound ? 'disabled' : ''} onchange="toggleBatchItemSelect(${idx}, this.checked)">
            </div>
            ...
            <div class="batch-item-actions">
                <button class="batch-item-delete" title="删除" onclick="deleteBatchItem(${idx})">
...
function toggleBatchItemSelect(idx, checked) {
    if (checked) {
        batchSelectedIndexes.add(idx);
    } else {
        batchSelectedIndexes.delete(idx);
    }

    const item = document.querySelector(`.batch-result-item[data-idx="${idx}"]`);
    if (item) {
        if (checked) {
            item.classList.add('batch-item-selected');
        } else {
            item.classList.remove('batch-item-selected');
        }
    }
...
async function batchDownloadToServer() {
    ...
    if (settingsDelayInput) {
        ...
        if (delayText !== '') {
            const parsedDelay = parseInt(delayText);
            if (!isNaN(parsedDelay)) delayValue = parsedDelay;
        }
    }
