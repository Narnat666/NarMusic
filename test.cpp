...
private:
    ...
    std::string adjustLyricsTiming(const std::string& lyrics, int offsetMs) {
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cout << "========================================" << std::endl;
        std::cout << "音乐文件元数据更新工具 v2.3" << std::endl;
        std::cout << "========================================" << std::endl;
        std::cout << "功能: 自动从多个音乐平台获取歌词和封面" << std::endl;
        std::cout << "支持平台: 网易云音乐、酷狗音乐、QQ音乐" << std::endl;
        std::cout << "数据策略:" << std::endl;
        std::cout << "  - 歌词: 优先使用酷狗音乐，其次网易云，最后QQ音乐" << std::endl;
        std::cout << "  - 封面: 从所有平台下载，选择文件最大的（质量最好的）" << std::endl;
        std::cout << "  - 基本信息: 优先使用酷狗音乐的信息" << std::endl;
        ...
        std::cout << "  3. 自动选择最高质量封面" << std::endl;
